...
**Method 3: Solving the inequality**
...
for (i in seq_along(n)) {
  ...
  for (j in seq_along(alpha_list)) {
    ...
    # method 3 interval
    lower <- sample_means / (1 + z / sqrt(ni))
    upper <- sample_means / (1 - z / sqrt(ni))

    coverage3[i,j] <- mean(lower <= theta & theta <= upper)
    lengths <- upper - lower

    avgLength3[i,j] <- mean(lengths)
    sdLength3[i,j] <- sd(lengths)
  }
}
```
...
for (j in 1:length(alpha_list)) {
  ...
  colnames(length_df) <- c("n", "M1 (Exact)", "M2 (Normal)", "M3 (Inequality)", "M4 (Student)")
  ...
  colnames(efficiency_df) <- c("n", "M1 (Exact)", "M2 (Normal)", "M3 (Inequality)", "M4 (Student)")
...
method_names <- c("Method 1 (Exact)", "Method 2 (Normal)",
                  "Method 3 (Inequality)", "Method 4 (Student)")
...
From Method 2 we have the approximate confidence statement
$$
P\left(|\bar{X} - \theta| \le z_{\alpha/2}\frac{\theta}{\sqrt{n}}\right) \approx 1 - \alpha.
$$

This inequality contains the unknown parameter \(\theta\) on both sides, so we solve it explicitly for \(\theta\).
The condition
$$
|\bar{X} - \theta| \le z_{\alpha/2}\frac{\theta}{\sqrt{n}}
$$
is equivalent to
$$
\bar{X} - z_{\alpha/2}\frac{\theta}{\sqrt{n}} \le \theta \le \bar{X} + z_{\alpha/2}\frac{\theta}{\sqrt{n}}.
$$

Rearranging the left inequality gives
$$
\theta \ge \frac{\bar{X}}{1 + \frac{z_{\alpha/2}}{\sqrt{n}}},
$$
and rearranging the right inequality gives
$$
\theta \le \frac{\bar{X}}{1 - \frac{z_{\alpha/2}}{\sqrt{n}}}.
$$

Thus we obtain the parameter-free confidence interval
$$
P\left(
\frac{\bar{X}}{1 + \frac{z_{\alpha/2}}{\sqrt{n}}}
\;\le\;
\theta
\;\le\;
\frac{\bar{X}}{1 - \frac{z_{\alpha/2}}{\sqrt{n}}}
...
1. **Coverage Accuracy:**
   ...
   - **Method 3 (Inequality)** achieves excellent coverage accuracy, often matching or even slightly exceeding Method 1's performance.
   For the 90% confidence level, it has the best average coverage accuracy (min error = 0.0024 across all n).
   - **Methods 2 and 3** both show very good coverage for all sample sizes, with convergence improving as $n$ increases,
   confirming the validity of the CLT and normal approximation for the exponential distribution.
   - **Method 4 (Student t)** shows noticeable under-coverage for small samples ($n = 30$), with errors around 2.0-2.4%,
   due to the skewness of the exponential distribution affecting the t-approximation. However, it converges to proper coverage
   as sample size increases.

2. **Interval Precision:**
   - **Method 1** produces the shortest intervals among all methods, serving as the benchmark for efficiency.
   - **Method 2** has slightly wider intervals than Method 1 (efficiency $\approx 0.94$-$0.99$) due to using the normal approximation
   rather than the exact distribution.
   - **Method 2** produces the shortest intervals (efficiency ≈ 0.91-0.99 relative to Method 1, meaning 1-9% shorter)
   because it uses the normal approximation. The efficiency gap narrows as sample size increases, converging toward Method 1
   as the chi-squared distribution approaches normality.
   - **Method 4** (Student t) produces intervals similar to Method 2 for moderate to large samples, but can be slightly wider
   for small samples due to the heavier tails of the t-distribution.

3. **Recommendation:**

**For exponential data specifically, I recommend Method 1 (Exact Chi-Squared)** because:
...
**For practical applications where you want a parameter-free method:**

**Method 3 (Inequality) is an excellent practical choice** because:

- It achieves proper coverage across all sample sizes and confidence levels
- The width penalty is modest (only 3-6% wider than optimal for n≥50)
- It requires no knowledge of the true parameter $\theta$, making it actually implementable in practice
- It converges rapidly to Method 1's efficiency as sample size increases
- It is algebraically simple and provides a closed-form solution
- Unlike Method 2, it can actually be computed from data alone

**For general statistical software or when the distribution is unknown:**

- Use **Method 4 (Student t)** - it's the most universal approach
- It works reasonably well for moderate to large samples ($n \geq 50$)
- It's conservative (maintains or slightly exceeds coverage)
- It's the standard approach in statistical practice for unknown distributions
- However, be cautious with small samples from skewed distributions like the exponential
...
**Summary:** For exponential data, Method 1 is theoretically optimal. However, **Method 3 stands out as the best practical compromise**,
offering near-optimal performance with only a small efficiency loss while being fully implementable from sample data alone.
Method 4 is the safe universal choice when the underlying distribution is unknown.
...
```
As we can see for all ns the   $$ \hat\sigma_{n-1}^2 $$  and  $$ \hat\sigma_{n}^2 $$ is really close to actual variance (in my example it's 4).  For example, for n = 1000 it's 3.987485 that is do close to 4. 

 $$ \hat\sigma_{n-1}^2 $$  is pretty closer to the real value of variance in each case, but there is a small difference with biases: for all of ns they are so close to 0 (the greater n is, the greater convergency to 0 is + for n-1 we have unbiased estimator), but in over case for n = 100 there is some small difference (we have a tendency to > n -> bias is closer to 0, but it's differ here)

The best result is in 4th case (n = 1000) the worst – 3rd (n = 100)There is theoretical proof that $$ \hat\sigma_{n-1}^2  $$ is unbiased:

$$
E(\hat\sigma_n^2) = E\!\left( \frac{1}{n} \sum_{i=1}^n (X_i - \bar X)^2 \right) = \frac{1}{n} E(S_{xx}) = \frac{1}{n}(n-1)\sigma^2
$$

$$ E(\hat\sigma_{n-1}^2) = E\!\left( \frac{1}{n-1} \sum_{i=1}^n (X_i - \bar X)^2 \right) = \frac{1}{n-1} E(S_{xx}) = \frac{1}{n-1}(n-1)\sigma^2 = \sigma^2 $$

$$ \text{Bias}(\hat\sigma_n^2) = E(\hat\sigma_n^2) - \sigma^2 = \frac{n-1}{n}\sigma^2 - \sigma^2 = -\frac{\sigma^2}{n},\text{biased}
$$

$$ \text{Bias}(\hat\sigma_{n-1}^2) = E(\hat\sigma_{n-1}^2) - \sigma^2 = \sigma^2 - \sigma^2 = 0, \text{unbiased}
$$

So the main point is that we can use $$ \hat\sigma_{n-1}^2 $$ as unbiased estimator
